...
const Products = () => {
	...
	const filtered = useMemo(() => {
		...
		// ✅ search — tarjima qilingan matn bo‘yicha qidiradi
		if (search.trim()) {
		...
		<section className='section-padding pt-28 bg-background min-h-screen'>
			<div className='container-atelier'>
				...
				<div className='flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-10'>
					...
					{/* Search + Sort */}
					<div className='flex flex-col sm:flex-row items-stretch sm:items-center gap-3 md:w-auto w-full md:justify-end'>
						<div className='relative w-full sm:w-56'>
							...
							<input
								...
								className='w-full pl-9 pr-4 py-2 rounded-full bg-secondary text-sm text-foreground placeholder:text-muted-foreground outline-none focus:ring-2 focus:ring-accent/30 transition-all'
							/>
						</div>
						...
						<select
							...
							className='w-full sm:w-44 px-4 py-2 rounded-full bg-secondary text-sm text-foreground outline-none cursor-pointer'>
				...
				{filtered.length === 0 && <p className='text-center text-muted-foreground mt-12'>{t('products.noResults')}</p>}
			</div>
		</section>
	);
};
